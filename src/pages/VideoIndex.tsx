...
const VideoIndex = () => {
  ...
  // For documentaries, show all items in the People of Moiz series
  const items = allItems;

  const isDocumentary = type === 'documentaries';

  const [selected, setSelected] = useState<number | null>(null);
  const [focusedDocIdx, setFocusedDocIdx] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (!isDocumentary || focusedDocIdx === null) return;
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') closeDocFocus();
    };
    window.addEventListener('keydown', handleKey);
    return () => window.removeEventListener('keydown', handleKey);
  }, [isDocumentary, focusedDocIdx, closeDocFocus]);

  if (!info) return <div className="min-h-screen pt-32 text-center font-body text-muted-foreground">Not found</div>;

  // Documentary immersive layout — supports multiple items in the series
  if (isDocumentary && items.length > 0) {
    return (
      <main className="min-h-screen pt-16">
        <section className="px-6 py-10 md:px-12 mood-brown">
          <div className="max-w-6xl mx-auto">
            <p className="font-body text-xs font-semibold tracking-[0.3em] uppercase text-accent mb-3">Series</p>
            <h1 className={`${info.headingClass} text-5xl md:text-7xl`}>{info.title}</h1>
            <p className="font-handwriting text-lg opacity-60 mt-3">{info.subtitle}</p>
          </div>
        </section>
        ...
        <div className="flex flex-col gap-1">
          {items.map((project, idx) => {
            ...
            const focused = focusedDocIdx === idx;

            return (
              <article
                key={project.id}
                className={`relative overflow-hidden cursor-pointer transition-all duration-[360ms] group ${focused ? 'z-20' : 'z-10'}`}
                style={{
                  height: focused ? 'auto' : '50vh',
                ...
                tabIndex={0}
                onKeyDown={(e) => e.key === 'Enter' && !focused && setFocusedDocIdx(idx)}
              >
                ...
                {!focused && (
                  <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
                    <div className="max-w-2xl">
                      <p className="font-body text-xs font-semibold tracking-[0.3em] uppercase text-accent mb-3">People of Moiz · Episode {idx + 1}</p>
