...
import { Shuffle, Pause, Play, Volume2, VolumeX } from 'lucide-react';
...
const HeroVideo = () => {
  ...
  return (
    <section className={`relative min-h-[100svh] w-full overflow-hidden ${grainOn ? 'film-grain' : ''}`}>
      <div
      ...
      <div className="absolute bottom-6 left-6 md:left-16 z-[4] flex items-center gap-3">
        <button onClick={() => crossfadeToNext()} className="bg-studio-white/10 backdrop-blur-sm text-studio-white p-2.5 rounded-full hover:bg-studio-white/20 transition-all border border-studio-white/10" aria-label="Next clip">
          <Shuffle size={14} />
        </button>
        <button onClick={togglePause} className="bg-studio-white/10 backdrop-blur-sm text-studio-white p-2.5 rounded-full hover:bg-studio-white/20 transition-all border border-studio-white/10" aria-label={isPaused ? 'Play' : 'Pause'}>
          {isPaused ? <Play size={14} /> : <Pause size={14} />}
        </button>
        <button onClick={toggleMute} className="bg-studio-white/10 backdrop-blur-sm text-studio-white p-2.5 rounded-full hover:bg-studio-white/20 transition-all border border-studio-white/10" aria-label={isMuted ? 'Unmute' : 'Mute'}>
          {isMuted ? <VolumeX size={14} /> : <Volume2 size={14} />}
        </button>
        <button onClick={() => setGrainOn(!grainOn)} className="handwritten text-xs text-studio-white/40 hover:text-studio-white/70 transition-colors ml-2">
